// cropvid-notebook.swift — crop Safari chrome + the black rounded-corner
// margin off the Notebook screen recordings and emit an audio-stripped
// H.264 .mp4 plus a poster .jpg. No trim, no speed change (2x stays the
// site's playbackRate). AVAssetReader/Writer so the bitrate is controlled.
// Mirrors tools/cropvid.swift (the Barclay tool); parameterized here for
// Notebook's "group A" crop box (01–04, 10), reused to restore 01 and 02.
//
// Usage: cropvid-notebook <input.mov> <output.mp4> <poster.jpg>
//
// The crop rect is in the recordings' native 3132x2008 space (group A:
// x62 y106, 1476x840 — same box already used for notebook-01..04,10 on
// this page). Output size equals the crop size — no additional downscale.

import AVFoundation
import AppKit
import CoreImage

let CROP = CGRect(x: 62, y: 106, width: 1476, height: 840) // native px, group A
let OUT_W = 1476
let BITRATE = 2_000_000

let a = CommandLine.arguments
guard a.count == 4 else {
    FileHandle.standardError.write("usage: cropvid-notebook <in.mov> <out.mp4> <poster.jpg>\n".data(using: .utf8)!)
    exit(2)
}
...
Task {
    do {
        ...
        guard let track = try await asset.loadTracks(withMediaType: .video).first else {
            throw NSError(domain: "cropvid-notebook", code: 1, userInfo: [NSLocalizedDescriptionKey: "no video track"])
        }
        ...
        guard reader.startReading() else { throw reader.error ?? NSError(domain: "cropvid-notebook", code: 4) }
        writer.startWriting()
        writer.startSession(atSourceTime: .zero)

        var savedPoster = false
        let q = DispatchQueue(label: "cropvid-notebook.encode")
